...
title: Alembic 开发中的思考与洞察
slug: alembic-development-insights
featured: true
draft: false
tags:
  - Alembic
...
虽然我没想清楚，但我的直觉是对的，我在试图让现在这个强大的 AI，给我留下有价值的内容。可能因为我的性格使然，当我第一次使用 AI 编程我就想到，这么厉害的能力，用完即弃是一种奢侈的美式浪费。于是诞生了现在的 Alembic，一个符合中式节约主义的项目。
...
### 做了什么
- 开始思考 Alembic 与 AI IDE（Cursor/Copilot）的关系
- 调研 AI 自动生成代码片段的可行性
...
**🔑 洞察 3：互补而非竞争——找到自己的位置**
当 Cursor/Copilot 席卷而来时，第一反应可能是恐慌。但深入分析后发现：AI IDE 擅长「智能生成」但难以「标准化」，Alembic 擅长「标准化」但缺乏「智能」。二者是互补关系，不是替代关系。这个认知让项目从「被 AI 干掉」的焦虑中解脱出来，转向「给 AI 做外挂」的定位。

**🔑 洞察 4：可执行的知识 vs 可阅读的文档**
通过对比 6 大业界方案（Notion/Docs-as-Code/Prompt Library/RAG/Backstage），发现它们的共同问题是**知识不可执行**。Notion 里写了规范，开发者不看照样违反。关键差异在于：Alembic 的知识单元（后来叫 Recipe）绑定了依赖、import、SPM target，是**可执行的工程资产**，不是可阅读的文档。
...
**🔑 洞察 14：冷启动是第一印象**
「用户第一次用 Alembic」的体验决定了他会不会继续用。双 Agent 冷启动（Analyst 分析项目结构 → Producer 生成 Recipe 候选）让空知识库能在 5~10 分钟内产出 20~40 个候选条目。第一印象如果是空空如也的知识库，用户就不会回来了。
...
这是整个项目最大胆的决策。Alembic 最初因 Xcode Snippet 而生，现在把这部分全删了。背后的逻辑是：当 AI Agent 通过 MCP 直接消费知识时，IDE 内的 Snippet 自动插入变得多余。与其维护两套交付通道（MCP + Snippet），不如全力做好一套。备份分支 `backup/full-platform-services` 是安全网，但事后证明不需要回滚。
...
Anthropic 的 AutoDream 在 session 结束后自动整理记忆（合并、修剪、重组）。这启发了 Alembic 的知识衰退机制：不再使用的知识应该自动衰减、过时的知识应该被检测和清理。知识库不能只增不减。
...
OpenClaw 189k stars，零治理组件。安全通过环境物理隔离实现，不是每个操作检查权限。Alembic 之前做了 6 角色 × 27 Action 的权限矩阵，实际只有 3 个入口。MCP 工具暴露了什么 = Agent 能做什么，它无法「漂移」到未暴露的能力。删了 ~1090 行死代码（RoleDriftMonitor、SessionManager、ComplianceEvaluator），换成三层：防御性编程（删除前备份）、项目配置（boxspec.json）、工具注册本身就是权限边界。
...
同一个项目需要两套完全不同的文档输出：一套给 AI 读（.mdc 压缩为 When/Do/Don't，token 极简），一套给人类读（Markdown 完整文章 + Mermaid 架构图）。`.cursor/` 通常 gitignore，而 `Alembic/wiki/` 可 Git 共享。数据来源几乎零重叠——Delivery 消费已提炼的知识，Wiki 消费原始代码结构。两者互补而非竞争。
...
### 主线二：在 AI 浪潮中找到自己的位置
不是替代 AI，不是被 AI 替代，而是**成为 AI 的外挂记忆**。AI 负责智能，Alembic 负责权威性和可执行性。
